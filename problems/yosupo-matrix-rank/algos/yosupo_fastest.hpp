#pragma once
// =============================================================================
// Source: yosupo "Rank of Matrix" 提出 285774 (cp-algo, https://cp-algorithms.com)
//   https://judge.yosupo.jp/submission/285774
//   方式: cp-algo の linalg::matrix<modint<P>>::rank()。N>M なら .T() で転置、
    ...
    struct matrix: std::vector<_vec_t> {
        ...
        std::optional<std::array<matrix, 2>> solve(matrix t) const {
            ...
            if(sols.n() < t.m() || sols.submatrix(
                std::views::drop(sols.n() - t.m()) | std::views::take(t.m()),
                std::views::drop(m()) | std::views::take(t.m())
            ) != -eye(t.m())) {
                return std::nullopt;
            } else {
                return std::array{
                    sols.submatrix(std::views::drop(sols.n() - t.m()) | std::views::take(t.m()),
                                   std::views::take(t.m())),
