...
constexpr array<array<u64, 256>, 8> FROB_BYTE= [] {
 ...
 for(int p= 0; p < 8; ++p) {
  for(int b= 0; b < 256; ++b) {
   u64 v= u64(b) << (8 * p);
   for(int i= 0; i < 63; ++i) v= sq(v);
   t[p][b]= v;
  }
 }
 return t;
}();
inline u64 sqrt(u64 a) { return FROB_BYTE[0][u8(a)] ^ FROB_BYTE[1][u8(a >> 8)] ^ FROB_BYTE[2][u8(a >> 16)] ^ FROB_BYTE[3][u8(a >> 24)] ^ FROB_BYTE[4][u8(a >> 32)] ^ FROB_BYTE[5][u8(a >> 40)] ^ FROB_BYTE[6][u8(a >> 48)] ^ FROB_BYTE[7][u8(a >> 56)]; }
}
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& as) {
  using gf2_64_frob16_linear_map::sqrt;
