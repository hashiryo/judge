#pragma once
// a^8 を F_2-線型写像と見て、 8 byte tables (16 KB) の lookup で計算。
//   a^8 = XOR_{p=0..7} FROB_BYTE[p][(a >> 8p) & 0xFF]
//
// init: 8 × 256 = 2048 個の sq^3(b << 8p) を runtime 計算。
// per query: 8 lookup + 8 XOR ≈ 8-12 cyc。
#pragma GCC optimize("O3,unroll-loops")
#include "../../_shared/_common.hpp"
namespace gf2_64_frob8_linear_map {
...
inline u64 frob8_naive(u64 a) {
 for(int i= 0; i < 3; ++i) a= sq_naive(a);
 return a;
}
...
inline void init_tables() {
 ...
 inited= true;
 for(int p= 0; p < 8; ++p)
  for(int b= 0; b < 256; ++b) FROB_BYTE[p][b]= frob8_naive(u64(b) << (8 * p));
}
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& as) {
  using namespace gf2_64_frob8_linear_map;
  init_tables();
