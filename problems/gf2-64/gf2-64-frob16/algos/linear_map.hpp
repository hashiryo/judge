...
//
// init: 8 × 256 = 2048 個の sq^4(b << 8p) を runtime 計算。
...
inline void init_tables() {
 ...
 inited= true;
 for(int p= 0; p < 8; ++p)
  for(int b= 0; b < 256; ++b) FROB_BYTE[p][b]= frob16_naive(u64(b) << (8 * p));
}
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& as) {
  using namespace gf2_64_frob16_linear_map;
  init_tables();
