...
// loop 用 2 並列 mul: GPR in / pair out。pow (target 属性なし) からの非 inline 呼び出しでも
// 引数 4 本と戻り値が全部レジスタ (rdi..rcx / rax:rdx) で済む形にする。
VPCLMUL inline pair<u64, u64> mul2s(u64 a0, u64 b0, u64 a1, u64 b1) {
 __m256i prod= _mm256_clmulepi64_epi128(_mm256_set_epi64x(0, a1, 0, a0), _mm256_set_epi64x(0, b1, 0, b0), 0);
 __m256i d_full= _mm256_xor_si256(prod, _mm256_slli_epi64(prod, 1));
 __m256i red1_full= _mm256_xor_si256(d_full, _mm256_slli_epi64(d_full, 3));
 __m256i red1_shift= _mm256_srli_si256(red1_full, 8);
 __m256i indices= _mm256_srli_si256(_mm256_srli_epi64(prod, 60), 8);
 __m256i result= _mm256_xor_si256(_mm256_xor_si256(prod, red1_shift), _mm256_shuffle_epi8(RED_TABLE, indices));
 return {u64(_mm256_extract_epi64(result, 0)), u64(_mm256_extract_epi64(result, 2))};
}
u64 pow(u64 a, u64 e) {
 if(!e) return 1;
 if(!a) return 0;
 ...
 const u64 rl= r & 0xFFFFFF, rh= r >> 24;
 const u64 b= embed_idx(TABLES.PW_SIGMA_IDX[(u32(TABLES.LN_SIGMA[u16(B)]) * q) % 65535]);
 const u64 bt= mul(b, T[rl & 0xF]);
 const int it= int(r >> 48) + 5;
 u64 gl= T[(rl >> (4 * it)) & 0xF], gh= T[(rh >> (4 * it)) & 0xF];
 for(int i= it - 1; i >= 1; --i) {
  gl= frob4(gl), gh= frob4(gh);
  tie(gl, gh)= mul2s(gl, T[(rl >> (4 * i)) & 0xF], gh, T[(rh >> (4 * i)) & 0xF]);
 }
 gl= frob4(gl), gh= frob4(gh);
 tie(gl, gh)= mul2s(gl, bt, gh, T[rh & 0xF]);
