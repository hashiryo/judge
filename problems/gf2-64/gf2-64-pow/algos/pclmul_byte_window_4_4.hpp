...
VPCLMUL inline void mul2_(const __m256i& a_vec, const __m256i& b_vec, u64& r0, u64& r1) {
...
u64 pow(u64 a, u64 e) {
 ...
 mul2_(_mm256_set1_epi64x(T[2]), T2a, T[3], T[4]);
 // L3: T[5..8]
 __m256i T4= _mm256_set1_epi64x(T[4]);
 __m256i T43= _mm256_set_epi64x(0, T[4], 0, T[3]);
 mul2_(T4, T2a, T[5], T[6]);
 mul2_(T4, T43, T[7], T[8]);
 // L4: T[9..15] (T[15] は単独 mul)
 __m256i T8= _mm256_set1_epi64x(T[8]);
 mul2_(T8, T2a, T[9], T[10]);
 mul2_(T8, T43, T[11], T[12]);
 mul2_(T8, _mm256_set_epi64x(0, T[6], 0, T[5]), T[13], T[14]);
 T[15]= mul(T[8], T[7]);
