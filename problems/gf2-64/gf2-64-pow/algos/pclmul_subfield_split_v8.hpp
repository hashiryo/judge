#pragma once
// pclmul_subfield_split_v7_2 のメイン loop 4-lane 分割版 (byte_window_6 の subfield 版):
//
// r = r_3·2^36 + r_2·2^24 + r_1·2^12 + r_0 と 12 bit ずつ 4 分割すると
//   a^r = frob36(a^{r_3}) · frob24(a^{r_2}) · frob12(a^{r_1}) · a^{r_0}
// 4 lane とも 3 nibble 固定 (init + 2 反復) の同型漸化式なので lockstep で回し、
// mul は mul2 ×2、frob4 は XOR 集約を __m256i で畳む frob4_4lane (packed → extract ×4)。
// 直列チェーン: v7_2 の init + 5 反復 → init + 2 反復 + 結合 (frob ×3 + mul2 + mul)。
//
// - 結合の frob12 / frob36 は frob.hpp に新設した byte table (frob24 は既存)
// - b = N^q は v7_2 と同じく lane0 の最下位 nibble operand に mul(b, T[r&15]) で織り込み
// - r ≥ 2^48 (確率 ~2^-16) のときだけ r_3 が 4 nibble なので反復 +1 (他 lane は先頭 0 詰め)
// - preprocessing (LN/PW constexpr 構築, T tree, B) は v7_2 と同一
#pragma GCC optimize("O3,unroll-loops")
#include "../../_shared/_common.hpp"
#include "../../_shared/sq.hpp"
#include "../../_shared/frob.hpp"
namespace gf2_64_pow_subfield_split_v8 {
...
// frob4 ×4 lane: packed (q0..q3) の ymm で XOR 集約してから 4 lane を取り出す
...
GNU_TARGET("pclmul,vpclmulqdq") u64 pow(u64 a, u64 e) {
 ...
 for(int i= it - 1; i >= 1; --i) {
  ...
  tie(A0, A1)= unpack(mul2(_mm256_set_epi64x(0, f1, 0, f0), _mm256_set_epi64x(0, T[(r1 >> (4 * i)) & 0xF], 0, T[(r0 >> (4 * i)) & 0xF])));
  tie(A2, A3)= unpack(mul2(_mm256_set_epi64x(0, f3, 0, f2), _mm256_set_epi64x(0, T[(r3 >> (4 * i)) & 0xF], 0, T[(r2 >> (4 * i)) & 0xF])));
 }
 // 最終段 (i=0): lane0 に bt (= b·T[r&15]) を織り込み
 auto [f0, f1, f2, f3]= frob4_4lane(A0, A1, A2, A3);
 tie(A0, A1)= unpack(mul2(_mm256_set_epi64x(0, f1, 0, f0), _mm256_set_epi64x(0, T[r1 & 0xF], 0, bt)));
 tie(A2, A3)= unpack(mul2(_mm256_set_epi64x(0, f3, 0, f2), _mm256_set_epi64x(0, T[r3 & 0xF], 0, T[r2 & 0xF])));
...
struct GF2_64Op {
 static vector<u64> run(const vector<u64>& as, const vector<u64>& es) {
  using gf2_64_pow_subfield_split_v8::pow;
