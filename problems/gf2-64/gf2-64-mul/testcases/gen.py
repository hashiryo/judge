...
def make_pairs(name: str, T: int, kind: str) -> list[tuple[int, int]]:
    ...
    if kind == "small":
        return [(rng.randint(0, 255), rng.randint(0, 255)) for _ in range(T)]
    if kind == "random":
        return [(rng.randint(0, MASK64), rng.randint(0, MASK64)) for _ in range(T)]
    if kind == "edge_zero_one":
        out = []
        for _ in range(T):
            choices = [0, 1, MASK64, rng.randint(0, MASK64)]
            out.append((rng.choice(choices), rng.choice(choices)))
        return out
    if kind == "structured":
        out = []
        for _ in range(T):
