...
struct BSGSTable6700417 {
 ...
 u32 solve(u64 target) const {
  u64 t[4];
  t[0]= target;
  t[1]= mul(t[0], inv_base_m);
  t[2]= mul(t[1], inv_base_m);
  t[3]= mul(t[2], inv_base_m);
  u64 t_n[4], t_n2[4];
  for(int j= 0; j < 4; ++j) t_n[j]= mul(t[j], inv4_base_m);
  for(int j= 0; j < 4; ++j) t_n2[j]= mul(t_n[j], inv4_base_m);
  for(int j= 0; j < 4; ++j) {
