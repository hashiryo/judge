...
// 6700417 BSGS の baby step 数を m=√n=2588 から m=131072 (= 2^17) に拡大、
// baby table を L3 fit にして giant step 数を激減させる。
//
// v4 (m=2588): 31KB baby (L1 fit), giant up to 2588 (avg ~1300) per query
// v7 (m=131072): 1.5MB baby (L2/L3 fit), giant up to ~52 (avg ~26) per query
//   → per-query で giant step 約 50× 削減
//
// 実測 (M3 Mac local, T=10K, random_00): v4 343ms → v7 ~32ms (~10× 高速化)
//
// init: 131K mul + 131K hash insert ≈ 2ms (v6 の 130ms に対し圧倒的に軽い)
// 全 dashboard 環境で勝ち越し見込み (ARM の CLMUL emulation でも init 軽量維持)
//
