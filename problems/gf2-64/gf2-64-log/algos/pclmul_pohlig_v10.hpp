...
u32 solve_f16(u64 x_proj_poly);  // 下で定義 (constexpr H_LOG_INV を使う)
...
//   h_poly    = gf_pow(2, EXP_F16)  → H_LOG_INV 計算用
...
// LN_BETA も constexpr なので H_LOG_INV まで compile-time に確定
constexpr u32 H_LOG_INV= u32(modinv_runtime(LN_BETA[u16(H_POLY)], 65535));
u32 solve_f16(u64 x_proj_poly) {
 const u32 idx= u16(x_proj_poly);
 if(idx == 0) return 0;
 const u32 log_x= LN_BETA[idx];
...
void init_tables() {
 if(inited) return;
 inited= true;
 // pow_bw 4 回 (G_2^EXP_*, H_POLY) は constexpr 値で置換済み → init は hash table 構築のみ
