#pragma once
// "Naive" 実装。yosupo_337840 と同じ同型変換 + reduce を使うが、
// 多項式乗算だけ PCLMUL を使わずに**ビット単位 XOR シフトのループ**で実装する。
// 単純な GF(2) 多項式乗算アルゴリズムの教科書版。
//
// 同じ algorithm 構造で「PCLMUL の効果がどれくらいあるか」を比較するための
// 対照実装。アーキ依存命令を使わないので arm でも動くが、64 bit 1 回の積で
// 64 回の XOR + シフトが要るので fastest より十数倍以上遅い見込み。
...
// yosupo_337840 と同じ基底変換テーブル。共通化はせず、独立に持って
// 「対照実験」を物理的に分離 (片方の最適化が他方に波及しないように)。
...
// 素朴 carryless multiply (GF(2) 上の多項式乗算)。64 回のループ。
inline std::pair<uint64_t, uint64_t> clmul_naive(uint64_t a, uint64_t b) {
 uint64_t lo = 0, hi = 0;
 for (int i = 0; i < 64; ++i) {
  if ((b >> i) & 1) {
   lo ^= a << i;
...
// reduce_mod (yosupo_337840 と同じだが __m128i 不使用)
inline uint64_t reduce_mod_pair(uint64_t lo, uint64_t hi) {
