#pragma once
// =============================================================================
// Source: yosupo "Power Projection of Set Power Series" 提出 339877 (cp-algo).
//   https://judge.yosupo.jp/submission/339877
//   方式: cp-algo の math::subset_power_projection<base>(g, w, M)。
//   subset_conv_transpose を半分再帰で重ねて、最深部で
//   pw[i][j] = pw[i-1][j-1] * j のテーブルを使い w を g^m と内積。
//   O((2^N · N + M) ぐらいに収束 (係数省略)。
// 抽出方針 (subset_convolution / subset_exp / subset_log と同じ):
...
//   - bitset 用 read_bits / write_bits は subset_power_projection で未使用なので削除
//   - 注意: subset_compose / subset_conv_transpose / subset_power_projection は
//           本問題で必要なので残す (前 3 問では削除していた)。
//           views::enumerate を使う部分があるが libc++ 18+ にあるので OK。
...
namespace cp_algo {
    ...
    // bitset 用の read_bits / write_bits 系は subset_power_projection 経路で
    // 未使用なので削除 (clang/simde で scalar→vector 変換に詰まるのも回避)。
}
#pragma GCC pop_options
...
namespace cp_algo::math {
    ...
    big_vector<base> subset_power_projection(big_vector<big_vector<base>> &&fg, std::span<base> g, size_t M) {
        ...
        size_t N = std::size(g);
        fg.emplace_back(N / 2);
        // 元コードは `fg | views::enumerate | views::reverse | views::drop(1)` だが
        // libc++ (macOS) には views::enumerate がまだ無いので展開して書く。
        // i = size(fg)-2 ..= 0 の逆順、各回 fg[i+1] を fg[i] から更新。
