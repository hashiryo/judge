...
#include "_common.hpp"
// =============================================================================
// maspy_o1_plantard.hpp の (MAGIC1, MAGIC2) を cache fit 寄りに調整した実験版。
//
// 制約: √MAGIC1 · MAGIC2 ≳ p、かつ Farey 分母 b ≤ 65535
//
// 採用: MAGIC1 = 256K (= 2^18), MAGIC2 = 2M (= 2^21)
//   √MAGIC1 = 512, 512 × 2M ≈ 1.07e9 > p (998244353) ✓
//   分母 b ≤ √MAGIC1 = 512 で 16-bit 余裕
//   memory: Farey 1MB (L2 fit!) + inv 8MB (LL のみ)
//   現状 (Farey 4MB + inv 5.2MB, 両方 LL) と比較し Farey lookup の L2 ヒットを狙う
//
// 試算: per-query で Farey lookup を ~30 cyc → ~10 cyc に短縮 = 20 cyc/query
//   T=1M で ~7ms 短縮見込み
// 代償: inv table が 5.2MB → 8MB に増加 (元から LL なので大差なし期待)
// =============================================================================
struct ModInv {
 using i32 = int32_t;
 static constexpr u32 MAGIC1 = 1u << 18;  // 262144
 static constexpr u32 MAGIC2 = 1u << 21;  // 2097152
