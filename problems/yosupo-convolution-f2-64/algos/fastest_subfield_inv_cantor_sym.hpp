...
VPCLMUL inline void nim_fft(std::vector<u64>& f) {
 ...
 while(len < n) {
  ...
  for(int l= 0; l < n; l+= len) {
   ...
   const u64* g0= g.data();  // g[i+half] = g0[i] ^ 1 なので g1 を読む必要なし
   u64* f0= f.data() + l;
...
VPCLMUL inline void nim_ifft(std::vector<u64>& f) {
 ...
 while(len > 1) {
  ...
  for(int l= 0; l < n; l+= len) {
   ...
   // tail: half == 1。 g0[0] = 0 なので s*g0 = 0, f0 = lo, f1 = s。
   for(; i < half; ++i) {
    u64 lo= f0[i], hi= f1[i];
    u64 s= lo ^ hi;
    u64 p= (g0[i] == 0) ? 0 : mul(s, g0[i]);
    f0[i]= lo ^ p;
    f1[i]= s;
   }
  }
