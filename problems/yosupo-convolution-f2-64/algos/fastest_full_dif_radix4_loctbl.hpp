#pragma once
#include "_common.hpp"
// fastest_full_dif_radix4.hpp の **per-level local-table 版**。
// (radix-4 + SUBMASK_TBL → stack の subs[16] build。)
...
namespace conv_f2_64_full_dif_radix4_loctbl {
...
inline const __m256i RED_TABLE_DIF_R4LOC= _mm256_setr_epi8(
...
VPCLMUL inline __m256i clmul_reduce_pair(__m256i a_vec, __m256i b_vec) {
 ...
 __m256i red_vec= _mm256_shuffle_epi8(RED_TABLE_DIF_R4LOC, indices);
...
inline void bc_to_lch(u64* poly, int n) {
 ...
 for(int level= d; level >= 2; --level) {
  ...
  int half= len >> 1;
  int sub_idx= level - 1;
  if(__builtin_popcount(sub_idx) == 1) {
...
struct Solver {
 VPCLMUL static std::vector<u64> run(int n, int m, const std::vector<u64>& a_in, const std::vector<u64>& b_in) {
  using namespace conv_f2_64_full_dif_radix4_loctbl;
