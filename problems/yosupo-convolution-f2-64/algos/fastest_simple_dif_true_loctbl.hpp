#pragma once
#include "_common.hpp"
// fastest_simple_dif_true.hpp の **per-level local-table 版**。
// SUBMASK_TBL constexpr 撤去、 bc_to_lch / bc_to_mono の outer level の冒頭で
// stack 上の int subs[16] に submask を build (level あたり 1 回)、
// hot inner では index lookup。 default (constexpr 焼き) と _notbl (inline while) の中間。
//
// 構造 (forward):
...
namespace conv_f2_64_simple_dif_true_loctbl {
...
inline void bc_to_lch(u64* poly, int n) {
 ...
 for(int level= d; level >= 2; --level) {
  ...
  int half= len >> 1;
  int sub_idx= level - 1;
  // per-level に local stack 上の subs[] を build (sub_idx ≤ 29 で popcount ≤ 4 → 最大 15 entries)
  int subs[16];
  int sub_n= 0;
  {
   int s= sub_idx;
 ...
 for(int level= 2; level <= d; ++level) {
  ...
  for(int base= 0; base < n; base+= len) {
   ...
   for(int i= 0; i < half; ++i) {
    u64 q= p[half + i];
    if(q == 0) continue;
    // submask 順序は inner で commutative (forward と同じで OK)
    for(int idx= 0; idx < sub_n; ++idx) p[i + subs[idx]]^= q;
   }
  }
 }
...
struct Solver {
 static std::vector<u64> run(int n, int m, const std::vector<u64>& a_in, const std::vector<u64>& b_in) {
  using namespace conv_f2_64_simple_dif_true_loctbl;
