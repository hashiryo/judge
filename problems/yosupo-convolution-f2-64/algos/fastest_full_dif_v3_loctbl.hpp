#pragma once
#include "_common.hpp"
// fastest_full_dif_v3.hpp の **per-level local-table 版**。
// (v3 = v2 + 4-way unroll、 さらに SUBMASK_TBL → stack の subs[16] build。)
//
// アルゴリズム:
...
namespace conv_f2_64_full_dif_v3_loctbl {
...
struct Solver {
 VPCLMUL static std::vector<u64> run(int n, int m, const std::vector<u64>& a_in, const std::vector<u64>& b_in) {
  using namespace conv_f2_64_full_dif_v3_loctbl;
