#pragma once
#include "_common.hpp"
// yosupo submission #369907 (https://judge.yosupo.jp/submission/369907) の移植。
// 当時 yosupo 2 番目の高速提出。
//
// アプローチ: nim (additive) FFT 系ではなく **Schönhage radix-3 アルゴリズム**。
//
...
public:
 ...
 [[gnu::target("pclmul,sse4.1")]] F_2_64& operator*=(const F_2_64& r) {
  ...
  static constexpr std::uint64_t t[]= {0, k, k << 1, k ^ (k << 1), k << 2, k ^ (k << 2), (k << 1) ^ (k << 2), k ^ (k << 1) ^ (k << 2)};
  high^= high << 1;
...
template<typename Tp> struct Radix3Schoenhage {
 ...
 static int Schoenhage(const Tp a[], const Tp b[], Tp ab[], int n) {
  ...
  for(int i= 0; i < delta * 2; ++i)
   for(int j= 0; j < d; ++j) a_hat[i * d * 2 + j]= a[i * d + j], b_hat[i * d * 2 + j]= b[i * d + j];
  FFT(data(a_hat), d, delta), FFT(data(b_hat), d, delta);
  int cnt;
  for(int i= 0; i < delta * 2; ++i) cnt= Schoenhage(data(a_hat) + i * d * 2, data(b_hat) + i * d * 2, data(ab_hat) + i * d * 2, d);
