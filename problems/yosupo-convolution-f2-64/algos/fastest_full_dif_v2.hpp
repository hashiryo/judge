...
namespace conv_f2_64_full_dif_v2 {
...
inline const __m256i RED_TABLE_DIF= _mm256_setr_epi8(
...
VPCLMUL inline __m256i clmul_reduce_pair(__m256i a_vec, __m256i b_vec) {
 ...
 __m256i red_vec= _mm256_shuffle_epi8(RED_TABLE_DIF, indices);
...
inline void bc_to_lch(u64* poly, int n) {
 ...
 for(int level= d; level >= 2; --level) {
  ...
  if(__builtin_popcount(sub_idx) == 1) {
   // 単一 proper submask 0、 つまり inner op は 1 つ: poly[base+i+1] ^= poly[base+half+i]
   for(int base= 0; base < n; base+= len) {
    u64* p= poly + base;
    for(int i= half - 1; i >= 0; --i) p[i + 1]^= p[half + i];
   }
  } else {
   // 一般 case: proper submasks を per-level 配列化
   int subs[16];
   int sub_n= 0;
   {
    int s= sub_idx;
...
struct Solver {
 VPCLMUL static std::vector<u64> run(int n, int m, const std::vector<u64>& a_in, const std::vector<u64>& b_in) {
  using namespace conv_f2_64_full_dif_v2;
