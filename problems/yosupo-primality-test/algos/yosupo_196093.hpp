#pragma once
// =============================================================================
// Source: yosupo "Primality Test" 提出 196093.
//   https://judge.yosupo.jp/submission/196093
//   方式: テーブル lookup + 段階的 base 数増加の Miller-Rabin。
//     - 32-bit (x < 2^32): 256 エントリの table から 1 base 引いて 1-base MR
//       ハッシュ: (x * 0xad625b89) >> 24
//     - 64-bit (x ≥ 2^32):
//         * 1 base 目: 2 (固定)
//         * 2 base 目: 16384 エントリ table から (x*0xad625b89)>>18 で引く
//         * x > 2^49 ならここで打ち切り (= 2-base MR)
//         * それ以外は 8 エントリ table から 3 base 目を引いて 3-base MR
//   Forišek-Jančina 系の table-MR をハッシュ + base 数のトレードオフで
//   段階的に減らす設計。テーブルは生 u16 リテラルで埋め込まれている。
// 抽出方針:
//   - 元提出は Atcoder Library 風の DynamicModint32/64 (Montgomery) の上に
//     乗っているが、依存連鎖を持ち込まず自前の Montgomery struct M32/M64 で
//     書き直す
//   - bases (256+16384+8) と算法本体はそっくりコピー
// =============================================================================
...
// 32-bit base 表 (256 エントリ, 単一 base のハッシュテーブル)。
...
inline bool isPrime32(uint32_t x) {
 if (x % 2 == 0 || x % 3 == 0 || x % 5 == 0 || x % 7 == 0 || x % 11 == 0 ||
     x % 13 == 0 || x % 17 == 0 || x % 19 == 0 || x % 23 == 0 || x % 29 == 0 ||
     x % 31 == 0 || x % 37 == 0 || x % 41 == 0 || x % 43 == 0)
  return x <= 43 && (x == 2 || x == 3 || x == 5 || x == 7 || x == 11 ||
                     x == 13 || x == 17 || x == 19 || x == 23 || x == 29 ||
                     x == 31 || x == 37 || x == 41 || x == 43);
 if (x < 47 * 47) return x > 1;
 M32 mo(x);
 const uint32_t h = x * 0xad625b89u;
 uint32_t d = x - 1;
 int s = __builtin_ctz(d);
 d >>= s;
 uint32_t one_m = mo.to_mont(1);
 uint32_t minus_one_m = mo.to_mont(x - 1);
 uint32_t cur = mo.to_mont(bases32[h >> 24]);
 // cur = base^d
 uint32_t result = one_m;
 uint32_t e = d;
 while (e) {
  if (e & 1) result = mo.mul(result, cur);
  cur = mo.mul(cur, cur);
  e >>= 1;
 }
 cur = result;
 if (cur == one_m) return true;
 while (--s) {
  if (cur == minus_one_m) return true;
  cur = mo.mul(cur, cur);
 }
 return cur == minus_one_m;
}

// 64-bit base 表 1 (16384 エントリ)。
  const static uint16_t bases[] = {
      1814,
      1307,
      1714,
      6838,
...
inline bool isPrime64(uint64_t x) {
 if (x < 4294967296ull) return isPrime32((uint32_t) x);
 if (x % 2 == 0 || x % 3 == 0 || x % 5 == 0 || x % 7 == 0 || x % 11 == 0 ||
     x % 13 == 0 || x % 17 == 0 || x % 19 == 0 || x % 23 == 0 || x % 29 == 0 ||
     x % 31 == 0 || x % 37 == 0 || x % 41 == 0 || x % 43 == 0)
  return false;
 M64 mo(x);
 uint64_t d = x - 1;
 const int s_orig = __builtin_ctzll(d);
 d >>= s_orig;
 uint64_t one_m = mo.to_mont(1);
 uint64_t minus_one_m = mo.to_mont(x - 1);
 auto pow_d = [&](uint64_t base_m) {
  uint64_t result = one_m;
  uint64_t b = base_m;
  uint64_t e = d;
  while (e) {
   if (e & 1) result = mo.mul(result, b);
   b = mo.mul(b, b);
   e >>= 1;
  }
  return result;
 ...
 // base 1: 2
 uint64_t cur = pow_d(mo.to_mont(2));
 if (cur != one_m) {
  int i = s_orig;
  while (--i) {
   if (cur == minus_one_m) break;
   cur = mo.mul(cur, cur);
  }
  if (cur != minus_one_m) return false;
 }
 // base 2: bases[hash(x)]
 const uint16_t base = bases[uint32_t(x * 0xad625b89u) >> 18];
 cur = pow_d(mo.to_mont(base));
 if (cur != one_m) {
  int i = s_orig;
  while (--i) {
   if (cur == minus_one_m) break;
...
struct Primality {
 static vector<bool> run(const vector<u64>& qs) {
  ...
  for (size_t i = 0; i < qs.size(); ++i)
   ans[i] = yosupo_196093::isPrime64(qs[i]);
  return ans;
 }
};
