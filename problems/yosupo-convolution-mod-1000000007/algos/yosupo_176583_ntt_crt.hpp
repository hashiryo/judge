...
#endif
// std::__lg は libstdc++ 拡張で libc++ には無いので補完。
#ifndef __GLIBCXX__
...
namespace simd
{
 ...
 // simde の NEON エミュレーションが imm を constant にしか受け付けないので
 // テンプレート化 (元のコードはランタイム int 引数だが、呼び出しは常に定数)。
...
struct NTT
{
 ...
 static void butterfly_forward_x4(u32 *addr_inp_0, u32 *addr_inp_1, u32 *addr_inp_2, u32 *addr_inp_3,
 ...
 static void butterfly_inverse_x4(u32 *addr_inp_0, u32 *addr_inp_1, u32 *addr_inp_2, u32 *addr_inp_3,
                                  u32 *addr_out_0, u32 *addr_out_1, u32 *addr_out_2, u32 *addr_out_3,
                                  u32x8 w_1, u32x8 w1, u32x8 w2, u32x8 w3,
                                  const Montgomery_simd &mts, u32x8 cum = u32x8())
 {
  ...
  if constexpr (!trivial || multiply_by_cum)
  {
   ...
   {
    store_u32x8(addr_out_0, mts.mul<false, true>(a1 + c1, cum));
   }
   store_u32x8(addr_out_1, mts.mul<false, true>(b1 + d1, w1));
   store_u32x8(addr_out_2, mts.mul<false, true>(a1 + mts.mod2 - c1, w2));
   store_u32x8(addr_out_3, mts.mul<false, true>(b1 + mts.mod2 - d1, w3));
  }
  else
  {
 ...
 static void butterfly_forward_x4(u32 *addr_0, u32 *addr_1, u32 *addr_2, u32 *addr_3,
                                  ...
                                  const Montgomery_simd &mts)
 {
  butterfly_forward_x4<trivial>(addr_0, addr_1, addr_2, addr_3, addr_0, addr_1, addr_2, addr_3, w_1, w1, w2, w3, mts);
 }
 ...
 static void butterfly_inverse_x4(u32 *addr_0, u32 *addr_1, u32 *addr_2, u32 *addr_3,
                                  ...
                                  const Montgomery_simd &mts, u32x8 cum = u32x8())
 {
  butterfly_inverse_x4<trivial, multiply_by_cum>(addr_0, addr_1, addr_2, addr_3, addr_0, addr_1, addr_2, addr_3, w_1, w1, w2, w3, mts, cum);
 }
 ...
 __attribute__((optimize("O3"))) typename std::enable_if<(lg >= 0), void>::type ntt_mul_rec_aux(int ind, u32 *data_a, u32 *data_b, Cum_info &cum_info, u32 fc) const
 {
  ...
  else
  {
       ...
       for (int i = 0, i0 = 0; i < (1 << lg); i += 4 * sz_it, i0++)
       {
        ...
        for (int j1 = j0, j2 = 0; j1 < sz_it; j1 += sz, j2++)
        {
         for (int j = j1; j < j1 + BLC; j += 8)
         {
          ...
          // std::array<u32 *, 4> bf = {buf + i0 * 4 * sz_bf + 0 * sz_bf + (j - j1) + j2 * BLC,
          //                            buf + i0 * 4 * sz_bf + 1 * sz_bf + (j - j1) + j2 * BLC,
          //                            buf + i0 * 4 * sz_bf + 2 * sz_bf + (j - j1) + j2 * BLC,
          //                            buf + i0 * 4 * sz_bf + 3 * sz_bf + (j - j1) + j2 * BLC};
          // auto inp = t == 0 ? dt : bf;
          // auto out = t == 2 ? dt : bf;
          // // inp = out = dt;
          // if (first && i == 0) {
          //     butterfly_forward_x4<true>(inp[0], inp[1], inp[2], inp[3],
          //                                out[0], out[1], out[2], out[3],
          //                                w_1, w1, w2, w3, mts);
          // } else {
          //     butterfly_forward_x4(inp[0], inp[1], inp[2], inp[3],
          //                          out[0], out[1], out[2], out[3],
          //                          w_1, w1, w2, w3, mts);
          // }
          if (first && i == 0)
   ...
   {
    ntt_mul_rec_aux<lg - RD, first>(ind, data_a, data_b, cum_info, fc);
    for (int i = 1; i < (1 << RD); i++)
    {
     ntt_mul_rec_aux<lg - RD, false>(ind + (1 << lg - RD) * i,
                                     data_a + (1 << lg - RD) * i,
                                     data_b + (1 << lg - RD) * i,
                                     cum_info, fc);
    }
