#pragma once
// =============================================================================
// Source: yosupo "Subset Convolution" 提出 352953 (cp-algo).
//   https://judge.yosupo.jp/submission/352953
//   方式: cp-algo の math::subset_convolution<base>(a, b)。
//   各 i ごとに長さ max_logn = 20 の rank vector を作って xor (Walsh-Hadamard)
//   transform、その後 rank 多項式積を AVX2 montgomery で計算 → 逆 transform。
//   メモリ局所性のため top/bottom に分割した 2 段 transform。
// 抽出方針:
//   - blazingio I/O / FastIO __io は使わない (base.cpp の cin で済ませる)
//   - <experimental/simd> include を削除 (libc++ stdfloat::float16_t 衝突回避)
//   - mmap path を Linux 限定に (MADV_HUGEPAGE/POPULATE_WRITE が macOS にない)
//   - __builtin_shuffle を clang では __builtin_shufflevector に分岐
//     (cp-algo 自身も rotate_right で同様にするが simd.hpp 末尾の rotate_right
//      は GCC 専用のままなので clang で分岐させる)
//   - #line マーカー除去
//   - SubsetConv::run wrapper で u32 入出力 ↔ big_vector<base> を変換
...
namespace cp_algo {
    ...
    // _pdep_u64 (BMI2) は subset_convolution 経路では未使用なので、
    // simde / 非 BMI2 ターゲットで未定義になっても落ちないよう guard。
#ifdef __BMI2__
    ...
    // bitset 用の read/write_bits 系は subset_convolution 経路で未使用。
    // simde や clang で scalar→vector 変換に詰まるので削除。
}
#pragma GCC pop_options
...
namespace cp_algo::math {
    ...
    // subset_exp / subset_compose / subset_conv_transpose / subset_power_projection は
    // 本問題 (subset_convolution) では未使用 + libc++ の views::enumerate / 大域 CTAD
    // で詰まるので抽出から落とす。
}
#pragma GCC pop_options
...
struct SubsetConv {
 static vector<u32> run(int N, const vector<u32>& a_in, const vector<u32>& b_in) {
  ...
  cp_algo::big_vector<base> a(sz), b(sz);
  for (size_t i = 0; i < sz; ++i) {
   a[i].setr((typename base::UInt) a_in[i]);
   b[i].setr((typename base::UInt) b_in[i]);
  }
  auto c = cp_algo::math::subset_convolution<base>(std::span<base>(a), std::span<base>(b));
