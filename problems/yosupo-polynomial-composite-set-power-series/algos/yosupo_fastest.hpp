#pragma once
// =============================================================================
// Source: yosupo "Polynomial Composite Set Power Series" 提出 339878 (cp-algo).
//   https://judge.yosupo.jp/submission/339878
//   方式: cp-algo の math::subset_compose<base>(f, g)。
//   subset_compose は半分再帰で f を変換、最深部で f に g[0] を冪展開して評価。
//   O(2^N * (M + N^2)) 程度。
// 抽出方針: subset_convolution / subset_exp / subset_log / power_projection と同様。
//   - blazingio / FastIO __io 不要 → 削除
...
//   - bitset 用 read_bits / write_bits 系を削除
//   - subset_compose の big_vector CTAD を big_vector<big_vector<base>> に明示
...
namespace cp_algo {
    ...
    // bitset 用の read_bits / write_bits 系は subset_compose 経路で未使用なので削除。
}
#pragma GCC pop_options
...
namespace cp_algo::math {
    ...
    big_vector<big_vector<base>> subset_compose(std::span<base> f, std::span<base> g, size_t n) {
        if (size(g) == 1) {
            ...
            // pw は元 M+1 だが、ループ中の `pw[i] = 0` で i が n-1 まで書くので
            // M < n-1 (例えば nm_01 ケースの M=0 や M=1) で OOB → SIGSEGV。
            // n を超える容量を確保しておく。
    ...
    big_vector<base> subset_power_projection(big_vector<big_vector<base>> &&fg, std::span<base> g, size_t M) {
        if (size(g) == 1) {
            ...
            // 同じく pw[i]=0 (i 最大 size(fg)-1) で OOB を避ける
            big_vector<base> pw(std::max(M + 1, n + 1));
            pw[0] = 1;
            for (size_t j = 1; j < M; j++) {
                pw[j] = pw[j - 1] * g[0];
        ...
        size_t N = std::size(g);
        fg.emplace_back(N / 2);
        // libc++ (macOS) には views::enumerate がまだ無いので手動 for に展開。
        for (int64_t i = (int64_t) size(fg) - 2; i >= 0; --i) {
            auto& h = fg[i];
...
struct PolyComp {
 static vector<u32> run(int M, int N, const vector<u32>& f_in, const vector<u32>& b_in) {
  ...
  cp_algo::big_vector<base> f((size_t) M);
  for (int i = 0; i < M; ++i) f[i].setr((typename base::UInt) f_in[i]);
  cp_algo::big_vector<base> g(sz);
  for (size_t i = 0; i < sz; ++i) g[i].setr((typename base::UInt) b_in[i]);
  auto c = cp_algo::math::subset_compose<base>(std::span<base>(f), std::span<base>(g));
